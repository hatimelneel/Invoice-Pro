...
function migrateBackupData(incoming){
  ...
  const cleanObjects=(value)=>Array.isArray(value)
    ? value.filter(x=>x && typeof x==="object" && !Array.isArray(x))
    : [];

  migrated.invoices=cleanObjects(migrated.invoices);
  migrated.quotes=cleanObjects(migrated.quotes);
  migrated.customers=cleanObjects(migrated.customers);
  migrated.products=cleanObjects(migrated.products);
  migrated.expenses=cleanObjects(migrated.expenses);
  migrated.customerPayments=cleanObjects(migrated.customerPayments);

  if(!migrated.settings || typeof migrated.settings!=="object" || Array.isArray(migrated.settings)){
    migrated.settings={};
  }

  migrated.invoices.forEach((x,i)=>{
    if(!x.number) x.number="INV-"+String(i+1).padStart(5,"0");
    if(x.customer===undefined) x.customer="";
    if(x.total===undefined) x.total=0;
    if(x.paid===undefined) x.paid=0;
    if(x.balance===undefined) x.balance=Math.max((+x.total||0)-(+x.paid||0),0);
    if(!Array.isArray(x.items)) x.items=[];
  });

  migrated.quotes.forEach((x,i)=>{
    if(!x.number) x.number="QT-"+String(i+1).padStart(5,"0");
    if(x.customer===undefined) x.customer="";
    if(x.total===undefined) x.total=0;
    if(!Array.isArray(x.items)) x.items=[];
  });
