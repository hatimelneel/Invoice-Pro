...
function renderPaper(d,type){
  ...
  const validityDays=Math.max(1,Math.round(+d.validity||30));
  const validUntil=new Date(d.date);
  validUntil.setDate(validUntil.getDate()+validityDays);
  const validUntilText=validUntil.toLocaleDateString(rtl?"ar-OM":"en-GB");
  ...
  paper.innerHTML=`
    <div class="paper-head">
      ...
      <div class="paper-title">${title}${type==="invoice" && (+d.balance===0 || d.paidFull)?`<span class="paid-badge">${tr("PAID","تم الدفع")}</span>`:""}${type==="quote"?`<span class="quote-validity-badge"><span class="qv-label">${tr("VALIDITY","صلاحية العرض")}</span><strong>${validityDays} ${tr(validityDays===1?"DAY":"DAYS","يوم")}</strong><small>${tr("Valid until","ساري حتى")}: ${validUntilText}</small></span>`:""}</div>
...
async function renderInvoiceCanvas(){
  ...
  // Very prominent quotation validity badge in the actual PDF canvas.
  if(previewType==="quote"){
    const badgeW=370,badgeH=92,badgeY=118,badgeX=rtl?right-badgeW:left;
    ctx.fillStyle=GOLD;
    ctx.beginPath();
    ...
    ctx.fillStyle=BLUE;
    ctx.textAlign="center";
    ctx.font="900 25px -apple-system, Arial";
    ctx.fillText(tr("VALIDITY","صلاحية العرض")+"  •  "+validityDays+" "+tr(validityDays===1?"DAY":"DAYS","يوم"),badgeX+badgeW/2,badgeY+37);
    ctx.font="700 19px -apple-system, Arial";
    ctx.fillText(tr("Valid until","ساري حتى")+" : "+validUntilText,badgeX+badgeW/2,badgeY+69);
  }
