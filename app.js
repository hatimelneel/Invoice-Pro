...
function normalizeDB(){
  ...
  db.customers=db.customers.filter(c=>c && typeof c==="object" && !Array.isArray(c));
  db.invoices=db.invoices.filter(x=>x && typeof x==="object" && !Array.isArray(x));
  db.quotes=db.quotes.filter(x=>x && typeof x==="object" && !Array.isArray(x));
  db.products=db.products.filter(x=>x && typeof x==="object" && !Array.isArray(x));
  db.expenses=db.expenses.filter(x=>x && typeof x==="object" && !Array.isArray(x));
  db.customerPayments=db.customerPayments.filter(x=>x && typeof x==="object" && !Array.isArray(x));

  db.customers.forEach(c=>{
    if(!c.id) c.id=makeCustomerId();
    c.name=txt(c.name);
    c.phone=txt(c.phone);
    c.address=txt(c.address);
  });

  db.invoices.forEach((inv,i)=>{
    inv.number=txt(inv.number)||("INV-"+String(i+1).padStart(5,"0"));
    inv.customer=txt(inv.customer);
    inv.phone=txt(inv.phone);
    inv.address=txt(inv.address);
    inv.customerId=txt(inv.customerId);
    inv.status=txt(inv.status)||"draft";
    inv.sourceQuoteNumber=txt(inv.sourceQuoteNumber);
    inv.notes=txt(inv.notes);
    inv.terms=txt(inv.terms);
    inv.date=txt(inv.date)||new Date().toISOString();

    if(!Array.isArray(inv.items)) inv.items=[];
    inv.items=inv.items.filter(x=>x && typeof x==="object").map(x=>({
      ...x,
      name:txt(x.name),
      qty:+x.qty||0,
      price:+x.price||0,
      isDescription:!!x.isDescription
    }));

    inv.total=+inv.total||0;
    inv.paid=+inv.paid||0;
    inv.balance=inv.balance===undefined
  ...
  db.quotes.forEach((q,i)=>{
    q.number=txt(q.number)||("QT-"+String(i+1).padStart(5,"0"));
    q.customer=txt(q.customer);
    q.phone=txt(q.phone);
    q.address=txt(q.address);
    q.customerId=txt(q.customerId);
    q.status=txt(q.status)||"draft";
    q.convertedInvoiceNumber=txt(q.convertedInvoiceNumber);
    q.notes=txt(q.notes);
    q.terms=txt(q.terms);
    q.date=txt(q.date)||new Date().toISOString();

    if(!Array.isArray(q.items)) q.items=[];
    q.items=q.items.filter(x=>x && typeof x==="object").map(x=>({
      ...x,
      name:txt(x.name),
      qty:+x.qty||0,
      price:+x.price||0,
  ...
  db.expenses.forEach(e=>{
    e.title=txt(e.title);
    e.documentType=txt(e.documentType);
    e.documentNumber=txt(e.documentNumber);
    e.amount=+e.amount||0;
    e.date=txt(e.date)||new Date().toISOString();
  });
  ...
  db.customerPayments
    ...
    .forEach(p=>{
      p.id=txt(p.id)||("PAY-"+Date.now()+"-"+Math.random().toString(36).slice(2,6));
      p.customerId=txt(p.customerId);
      p.customerName=txt(p.customerName);
      p.customerPhone=txt(p.customerPhone);
      p.date=txt(p.date)||todayLocalISODate();
      p.method=txt(p.method)||"cash";
      p.reference=txt(p.reference);
      p.note=txt(p.note);
      p.amount=+p.amount||0;
      if(!Array.isArray(p.allocations)) p.allocations=[];
      p.allocations=p.allocations.filter(a=>a && typeof a==="object").map(a=>({
        invoiceNumber:txt(a.invoiceNumber),
        amount:+a.amount||0
      }));
      ...
      if(!p.receiptNo){
        while(db.customerPayments.some(x=>x!==p && String(x.receiptNo||"")==="RCPT-"+String(receiptCounter).padStart(5,"0"))){
          receiptCounter++;
        }
        p.receiptNo="RCPT-"+String(receiptCounter++).padStart(5,"0");
      }else{
        p.receiptNo=txt(p.receiptNo);
      }
    });

  // Settings from older versions may also contain numeric values.
  db.settings.company=txt(db.settings.company);
  db.settings.phone=txt(db.settings.phone);
  db.settings.email=txt(db.settings.email);
  db.settings.address=txt(db.settings.address);
  db.settings.vat=txt(db.settings.vat);
  db.settings.currency=txt(db.settings.currency)||"OMR";
}
normalizeDB();
...
function show(id){
  ...
  if(id==="documents"){
    if(currentDocumentTab!=="invoices" && currentDocumentTab!=="quotes") currentDocumentTab="invoices";
    setDocumentTab(currentDocumentTab);
  }
...
function setDocumentTab(tab){
  currentDocumentTab=(tab==="quotes")?"quotes":"invoices";

  const invBtn=document.getElementById("docInvoicesBtn");
  const quoteBtn=document.getElementById("docQuotesBtn");

  if(invBtn) invBtn.classList.toggle("active",currentDocumentTab==="invoices");
  if(quoteBtn) quoteBtn.classList.toggle("active",currentDocumentTab==="quotes");
...
function renderDocuments(){
  ...
  try{
    if(currentDocumentTab!=="quotes" && currentDocumentTab!=="invoices"){
      currentDocumentTab="invoices";
    }

    const source=currentDocumentTab==="quotes"
        ...
        ? source.map((x,i)=>`
          <div class="document-row">
            <div class="document-main">
              <strong>${esc(x.number||"")}</strong>
              <small>${esc(x.customer||"")}</small>
              ${statusBadge(quoteDisplayStatus(x||{}))}
            </div>
            <div class="document-side">
              <strong>${money(x.total||0)}</strong>
              ${x.convertedInvoiceNumber?`<small class="converted-ref">${tr("Converted to","تم التحويل إلى")} ${esc(x.convertedInvoiceNumber)}</small>`:""}
              <div class="document-row-actions">
                <button onclick="openSavedQuote(${i})">${tr("Preview","معاينة")}</button>
                <button onclick="editSavedQuote(${i})">${tr("Edit","تعديل")}</button>
                ${x.convertedInvoiceNumber
                  ?`<button class="converted" onclick="openConvertedInvoice('${esc(x.convertedInvoiceNumber)}')">${tr("Open Invoice","فتح الفاتورة")}</button>`
    ...
    console.error("renderDocuments failed:",e);
    list.innerHTML=`<div class="empty">${tr(
      "Could not display restored documents. Please reload once.",
      "تعذر عرض المستندات المستعادة. أعد تحميل الصفحة مرة واحدة."
    )}</div>`;
  }
}
...
function validateBackupData(data){
  ...
  // Recognize Invoice Pro databases from old and new versions.
  const knownKeys=[
    "invoices","quotes","customers","products","expenses",
    "customerPayments","settings"
  ];
  return knownKeys.some(k=>Object.prototype.hasOwnProperty.call(data,k));
}
...
function parseBackupJSON(text){
  ...
  // Handle files that were accidentally JSON-stringified twice.
...
function extractBackupData(parsed){
  ...
  let incoming=parsed.data!==undefined ? parsed.data : parsed;

  // Some old exports may contain data as a JSON string.
  if(typeof incoming==="string"){
    incoming=parseBackupJSON(incoming);
  }

  // Be tolerant of one extra wrapper used by experimental builds.
  if(incoming && typeof incoming==="object" && incoming.data &&
     !Object.prototype.hasOwnProperty.call(incoming,"invoices")){
    incoming=incoming.data;
    if(typeof incoming==="string") incoming=parseBackupJSON(incoming);
  }

  if(!validateBackupData(incoming)) throw new Error("INVALID_BACKUP");
  return incoming;
}

function migrateBackupData(incoming){
  const migrated=(incoming && typeof incoming==="object")
  ...
  migrated.invoices=clean(migrated.invoices);
  migrated.quotes=clean(migrated.quotes);
  migrated.customers=clean(migrated.customers);
  migrated.products=clean(migrated.products);
  migrated.expenses=clean(migrated.expenses);
  migrated.customerPayments=clean(migrated.customerPayments);
...
function backupContentsSummary(data){
  return {
    ...
    payments:(data.customerPayments||[]).length
  };
}


function refreshAppAfterRestore(){
  currentDocumentTab="invoices";

  // Rebuild every screen from the newly restored database.
  populate();
  renderHome();
  renderInvoices();
  renderQuotes();
  ...
  if(document.getElementById("profitReports")){
    try{ renderProfitReport(); }catch(e){ console.error(e); }
  }
  if(document.getElementById("paymentLedger")){
    try{ renderPaymentLedger(); }catch(e){ console.error(e); }
  }

  // Re-bind the document-tab clicks explicitly for iOS/Safari.
  const invBtn=document.getElementById("docInvoicesBtn");
  const quoteBtn=document.getElementById("docQuotesBtn");
  if(invBtn) invBtn.onclick=()=>setDocumentTab("invoices");
  if(quoteBtn) quoteBtn.onclick=()=>setDocumentTab("quotes");
}


function saveRestoredDBSafely(){
  const originalLogo=db.settings&&db.settings.logo;

  try{
    localStorage.setItem(KEY,JSON.stringify(db));
    return {ok:true,logoRemoved:false};
  }catch(firstError){
    console.error("Initial restored DB save failed:",firstError);

    // If a very large embedded logo causes Safari localStorage quota trouble,
    // preserve all accounting data and fall back to the bundled/default logo.
    if(originalLogo && typeof originalLogo==="string" && originalLogo.startsWith("data:")){
      try{
        delete db.settings.logo;
        localStorage.setItem(KEY,JSON.stringify(db));
        return {ok:true,logoRemoved:true};
      }catch(secondError){
        db.settings.logo=originalLogo;
        console.error("Fallback restored DB save failed:",secondError);
        throw secondError;
      }
    }
...
function importBackupFile(event){
  ...
  if(file.size>0 && file.size<80){
    alert(tr(
      "This backup file is incomplete. Please choose the larger Invoice Pro JSON backup file.",
      "ملف النسخة الاحتياطية غير مكتمل. اختر ملف Invoice Pro JSON الأكبر حجماً."
    ));
    input.value="";
    return;
  }
  ...
  reader.onerror=()=>{
    alert(tr(
      ...
      "تعذر قراءة ملف النسخة الاحتياطية المحدد."
    ));
    input.value="";
  };
  ...
  reader.onload=()=>{
    ...
    // Only file parsing/validation is inside this error handler.
    try{
      parsed=parseBackupJSON(reader.result);
      incoming=migrateBackupData(extractBackupData(parsed));
      summary=backupContentsSummary(incoming);
    }catch(e){
      console.error("Backup parse error:",e);
      alert(tr(
        "This file is not a valid Invoice Pro backup.",
        "هذا الملف ليس نسخة احتياطية صالحة من Invoice Pro."
      ));
      input.value="";
      return;
    }
    ...
    const details=tr(
      `${summary.invoices} invoices, ${summary.quotations} quotations, ${summary.customers} customers, ${summary.payments} payments`,
      `${summary.invoices} فاتورة، ${summary.quotations} عرض سعر، ${summary.customers} عميل، ${summary.payments} دفعة`
    );

    if(!confirm(tr(
      "Restore "+details+"? Current data will be replaced.",
      "استعادة "+details+"؟ سيتم استبدال البيانات الحالية."
    ))){
      input.value="";
      return;
    }

    // Save only. Do not rebuild any screen while the old DOM is still active.
    try{
      db=incoming;
      normalizeDB();
      const saveResult=saveRestoredDBSafely();

      const backupLang=parsed && typeof parsed==="object" ? parsed.language : "";
      if(backupLang==="ar" || backupLang==="en"){
        localStorage.setItem("invoicepro_lang",backupLang);
      }

      if(typeof AUTO_BACKUP_DAY_KEY!=="undefined"){
        localStorage.setItem(AUTO_BACKUP_DAY_KEY,todayLocalISODate());
      }

      localStorage.setItem("invoicepro_restore_completed","1");
    }catch(e){
      console.error("Backup save error:",e);
      alert(tr(
        "The backup was read, but the data could not be saved.",
        "تمت قراءة النسخة، لكن تعذر حفظ البيانات."
      ));
      input.value="";
      return;
    }
    ...
    alert(tr(
      saveResult&&saveResult.logoRemoved
        ?"Backup restored successfully. The custom logo was too large for Safari storage, so the default logo will be used. The app will reopen now."
        :"Backup restored successfully. The app will reopen now.",
      saveResult&&saveResult.logoRemoved
        ?"تمت استعادة النسخة بنجاح. كان ملف الشعار كبيراً على تخزين Safari، لذلك سيستخدم التطبيق الشعار الافتراضي. سيتم فتح التطبيق من جديد الآن."
        :"تمت استعادة النسخة الاحتياطية بنجاح. سيتم فتح التطبيق من جديد الآن."
    ));

    // Full clean navigation to the same GitHub Pages path.
